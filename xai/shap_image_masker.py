...
class Takeishi_Image(Masker):
    """
    Adapted shap image masker from shap.maskers._image.Image
    This masks out image regions:
    instead of blurring or inpainting, we are using Takeishi's method,
    optimizing the input according to the output of the model.
    """

    def __init__(self, mask_value, model, device, shape=None, batch_mode=False):
        ...
        model: torch model to pass to the Takeishi method for optimizing inputs with pytorch
        ...
        self.model = model
        self.device = device
        self.mask_in_batch_mode = batch_mode
    ...
    def __call__(self, mask, x):
        ...
        # batch mode: we need to repeat x
        if self.mask_in_batch_mode:
            x = np.tile(x, (mask.shape[0], 1))
        else:
            x = np.expand_dims(x, 0)
            mask = np.expand_dims(mask, 0)
        # self.model = self.model.to(self.model.trainer.device)

        # NEWCODE: replacing all in-painting options by the Takeishi optimization call
        out = optimize_local_input_gradient_descent(data_point=x.reshape(x.shape[0], *in_shape),
                                                    mask=mask.reshape(mask.shape[0], *in_shape),  # 0 means mask, 1 means keep
                                                    predict_fn=self.model.forward,
                                                    max_iter=100,
                                                    lr=0.001,
                                                    gamma=0.01,
                                                    flip_channels=True,
